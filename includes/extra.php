	
<div class="services__wrap">
	<div class="container">
		<div class="row">

			<div class="col-md-12">
				<h1>Дополнительные услуги</h1>
			</div>
			<div class="clearfix"></div>		

			<div class="col-md-4">
				<div class="services services__view">
					<div class="services__item">
						<img src="img/p1.png" alt="">
					</div>
					<div class="services__item">
						<p>Консультирование клиентов по выбору конвейера с выездом специалиста на территорию заказчика
						</p>
					</div>
				</div>
			</div>

			<div class="col-md-4">
				<div class="services services__view">
					<div class="services__item">
						<img src="img/p2.png" alt="">
					</div>
					<div class="services__item">
						<p>Составление технического задания и произведение необходимых замеров</p>
					</div>
				</div>
			</div>

			<div class="col-md-4">
				<div class="services services__view">
					<div class="services__item">
						<img src="img/p3.png" alt="">
					</div>
					<div class="services__item">
						<p>Проведение всех необходимых подготовительных работ по установке транспортера
						</p>
					</div>
				</div>
			</div>
			<div class="clearfix"></div>


			<div class="col-md-4">
				<div class="services services__view">
					<div class="services__item">
						<img src="img/p4.png" alt="">
					</div>
					<div class="services__item">
						<p>Планирование и проектирование будущего оборудования</p>
					</div>
				</div>
			</div>

			<div class="col-md-4">
				<div class="services services__view">
					<div class="services__item">
						<img src="img/p5.png" alt="">
					</div>
					<div class="services__item">
						<p>Гарантийное и постгарантийное обслуживание</p>
					</div>
				</div>
			</div>

			<div class="col-md-4">
				<div class="services services__view">
					<div class="services__item">
						<img src="img/p6.png" alt="">
					</div>
					<div class="services__item">
						<p>Установка и ввод в эксплуатацию</p>
					</div>
				</div>
			</div>
			<div class="clearfix"></div>

		</div>
	</div>
</div>